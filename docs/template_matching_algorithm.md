...
粗层的同一空间峰最多保留两个角度模态：最高分方向，以及与它至少相差 90° 的
最高分方向。遮挡可能令错误的 180° 假设在粗层暂时领先，只保留一个方向会使正确
姿态无法到达 L0；不限制模态数量又会显著增加精匹配耗时。逐层精化的角度半径按
上一层采样步长加一个两度量化余量计算，而不是固定为 4°，确保候选能从粗量化
角度收敛到原图上的真实方向。
...
目标标签保持水平并优先位于旋转框上方，格式为
`#<index> T<template_id> S<score> M<scale> C<x>,<y>,<angle>`。标签使用与外包围框
一致的彩色底纹及白色描边文字；候选位置按上、下、左、右顺序选择并尽量避开已有
标签，靠近图像边缘时自动回退到可见区域。字体、内边距、线宽和左上角统计底纹均
随图像短边缩放。外包围框调色板明确排除红色和绿色，以免与逐点质量颜色混淆；
模板金字塔的 canonical 特征固定使用绿色。
